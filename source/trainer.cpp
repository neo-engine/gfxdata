...
int main( int p_argc, char** p_argv ) {
    ...
    for( const auto& [ i, img ] : images ) {
        auto fname = string( FSROOT "/" ) + string( p_argv[ 2 ] )
                     + to_string( i / MAX_ITEMS_PER_DIR ) + "/" + to_string( i ) + ".raw";
        fs::create_directories( string( FSROOT "/" ) + string( p_argv[ 2 ] )
                                + to_string( i / MAX_ITEMS_PER_DIR ) + "/" );
        ...
        printImage( f, fname, img, hg, wd, 1, THRESHOLD, false, 0, true );
        fclose( f );
    }
